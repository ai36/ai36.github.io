...
Иногда после сборки проекта, например на React, сборщик, вместо названия класса вставляет undefined, т.к. в объекте, в который импортируются стили компонента нет элемента с указанным названием. Это может произойти, если CSS-файл компонента не существует по указанному пути, либо указанный селектор отсутствует в CSS-файле компонента.
