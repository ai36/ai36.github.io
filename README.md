...
Запустив так или иначе это код, вы получите в консоли, что-то подобное этому:
```
<h3 class="_title_srkvi_31">                                debugger eval code:4:13
Object { width: 526, height: 31.916671752929688 }
```
Рядом с html-тегом размещена ссылка, клик по которой перемещается вас к нужному элементу в инспекторе кода в Dev tools браузера.
