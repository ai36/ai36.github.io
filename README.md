...
Обратите внимание, что спецификация не требует указывать полное время в формате UTC, можно ограничится только датой или только временем, но разделители и порядок элементов в UTC-строке должны быть валидными. К сожалению debug-снипет может проверить только его наличие атрибута, но его корректность его заполнения.

10. Есть изображения без атрибута width

Проверяется наличие у тегов `<img>` атрибута `widht`, которых отвечает за размер изображения в ширину. Это позволяет браузеру сразу выделить место для изображения, не дожидаясь обработки CSS. Это нужно, чтобы предотвратить или снизить сдиг элементов, после того, как CSS обработается браузером. Наличие/отстутсиве сдвига -  одна из важных метрик Core Web Vitals, подробнее [в этой статье](https://web.dev/articles/cls).
```html
...
По вышеуказанным причинам использовать атрибута `alt` без значения, с пробелами вместо значения или бессмысленным значением лишает ваш html-документ каких либо преимуществ. На данном этапе debug-снипет отследивает только наличие атрибута, но не его содержимое
...
15. Необходимо проверить alt у изображений, т.к. он предположительно содержит только пробелы
